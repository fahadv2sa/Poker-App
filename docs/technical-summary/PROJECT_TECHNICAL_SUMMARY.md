# Football Poker (فوتبول بوكر) — Project Technical Summary

- **Project name:** Football Poker (`football-poker` / repo `Poker-App`)
...
|---|---|---|
| `apps/web` | `@fp/web` | Next.js 15 (App Router) — all REST/SSR (auth, profile, stats, bank, rooms, social, install-reward, level-up) **and** the RTL table UI + Socket.IO client. Stateless; horizontally scalable. |
| `apps/game-server` | `@fp/game-server` | Node + Socket.IO **authoritative** game server: in-memory room state, the state machine, in-process turn/claim timers, all money writes inside DB transactions, Quick Play matchmaking, crash recovery, and the (flagged) Quick Play bots. **Must run as a single instance (replicas = 1).** |
| `packages/db` | `@fp/db` | Prisma schema + migrations + seed + the wallet/bank primitives + stats aggregation + the data import/score scripts. The only package that touches the DB. |
| `packages/engine` | `@fp/engine` | The pure Football Hand Engine + betting/pots/fold/resolve math. **No I/O, no DB, no clock.** Fully unit-testable. |
| `packages/shared` | `@fp/shared` | Enums, constants, Zod contracts, the WebSocket event contracts (`ws.ts`), the 9-rank catalog + Rule DSL (`handRanks.ts`), the badge catalog (`badges.ts`), profile helpers, the rate limiter. Single source of truth for cross-process types. |
...
**Game server (`@fp/game-server`):**
- Node 20 (engines pin; README notes it also runs on 24)
- `socket.io ^4.8.0`, `tsx ^4.19.1` (runs the TypeScript source directly — no build step at deploy), `jose ^6.2.3`, `@node-rs/argon2 ^2.0.2` (room-password verify), zod via `@fp/shared`
...
- **web service:** Build `pnpm --filter @fp/web build`; Start `pnpm --filter @fp/web start`; replicas may scale (stateless). Vars: `DATABASE_URL`, `DIRECT_URL`, `AUTH_SECRET`, `AUTH_URL`, `NEXT_PUBLIC_GAME_SERVER_URL` (baked at **build** time).
- **game-server service:** Build (none — `tsx` runs the TS source); Start `pnpm --filter @fp/game-server start` (→ `tsx src/index.ts`); **replicas = EXACTLY 1** (in-memory room state + timers). Vars: `DATABASE_URL`, `DIRECT_URL`, `AUTH_SECRET`, `WEB_ORIGIN`, and (operationally) `BOTS_ENABLED`. **Do not set `PORT`** (Railway injects it; the server binds `process.env.PORT` with a local fallback).
...
| Script | Action |
|---|---|
| `postinstall` | `pnpm --filter @fp/db generate` — regenerate the Prisma client (runs on every install/deploy; client is in a git-ignored path). |
...
**`@fp/web`:** `dev` (`next dev`), `build` (`next build`), `start` (`next start`), `lint`, `typecheck` (`tsc --noEmit`), `test` (`vitest run`).

**`@fp/game-server`:** `dev` (`tsx watch --env-file=.env.local src/index.ts`), `build` (`tsc`), `start` (`tsx src/index.ts`), `typecheck`, `test` (`vitest run`), `smoke` (`vitest run -c vitest.smoke.config.ts` — full hand against a live server), `seed:bots`.

**`@fp/db`:** `generate`, `migrate:dev`, `migrate:deploy`, `migrate:reset`, `seed`, `seed:players`, `import:api-football`, `import:tournament-stats`, `diff-import`, `transliterate:names`, `calculate:scores`, `studio` (`prisma studio`), `typecheck`, `test`, `test:watch`.

**`@fp/engine`:** `typecheck`, `test`.
**`@fp/shared`:** `typecheck`, `lint` (both `tsc --noEmit`).
...
pnpm --filter @fp/game-server dev          # Socket.IO :4000
pnpm --filter @fp/web dev                  # Next.js :3000
```

Open http://localhost:3000, register (1000 Coins), create a room, share the invite code. **Dealing needs players in the DB** — import/seed football players first or the deal fails (see §13.4). Quality gates: `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm --filter @fp/web build`. The wallet/bank suites hit a real PostgreSQL (start Postgres + `pnpm db:deploy`, or set `TEST_DATABASE_URL`).
...
- The HandRank catalog in `@fp/shared` is the single source of truth: the seed writes it to the DB and the engine interprets the same DSL — edit there and re-seed to change behavior.
