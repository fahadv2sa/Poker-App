/**
 * Pure round state machine (brief §4). The game-server owns the timers, the socket
 * I/O, the DB writes, and the choice of hint target; this module owns the LOGIC —
 * turn rotation, the two-rotations→HINT trigger, the hint cycle, reveals, scoring,
 * and round-end. Every function is pure: (state, event) → { state, events }.
 *
 * Seats are identified by their seat number. `hidden`/reveal `rank` are 1..10.
 */
...
export type Outcome =
  ...
  | { type: "wrong" }
  | { type: "already" }; // picked an already-revealed player — no penalty
...
export function normalGuess(state: RoundState, seat: number, outcome: Outcome): Step {
  ...
  if (outcome.type === "already") {
    // no penalty, no turn pass — keep guessing within the timer
