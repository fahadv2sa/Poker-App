...
function RoomCard({ r }: { r: RoomCardData }) {
  ...
  // friends rooms join by their invite code.
  const href = isPublic ? "/play" : r.code ? `/play?join=${encodeURIComponent(r.code)}` : "/play";
  ...
  return (
    <div className="relative z-10">
      ...
      <div className="grid gap-3">
        {rooms.length === 0 ? (
          tab === "public" ? (
            <EmptyState glyph="⚡" text="لا توجد طاولات سريعة نشطة الآن" cta={{ href: "/play", label: "ابدأ لعبًا سريعًا" }} />
          ) : (
            <EmptyState glyph="🔒" text="لا توجد غرف أصدقاء مفتوحة" cta={{ href: "/create-room", label: "أنشئ غرفة" }} />
          )
        ) : (
