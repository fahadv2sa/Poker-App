import { TrophyIcon, cn } from "@fb/top-10-ui";
import { LuHeader, LuScreen } from "./lu-screen";

/**
 * Top Ten rank leaderboard — a faithful copy of Link Up's RankView (podium + list),
 * fitted to Top Ten (its own gold screen chrome + level/xp from ttProgression). Pure
 * presentation: the ranked rows are computed in the page and passed in.
 */
export type TtRankedPlayer = {
  id: string;
  name: string;
  playerNumber: number;
  level: number;
...
function Podium({ p, place }: { p: TtRankedPlayer; place: 1 | 2 | 3 }) {
...
function RankRow({ p }: { p: TtRankedPlayer }) {
  return (
    <li
      className={cn(
...
export function TenRankView({
  podium,
  listed,
  pinnedMe,
  empty,
}: {
  podium: Array<{ p: TtRankedPlayer; place: 1 | 2 | 3 }>;
  listed: TtRankedPlayer[];
  pinnedMe: TtRankedPlayer | null;
  empty: boolean;
}) {
  return (
    <LuScreen>
