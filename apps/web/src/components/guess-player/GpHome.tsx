...
/**
 * Guess the Player — game home. Mirrors Top Ten's launcher one-for-one
 * (atmosphere + hero orb + action grid + rank strip + bottom nav) with this
 * game's routes and copy. Visual layer only.
 */

const ACTIONS = [
  { label: "إنشاء غرفة", href: "/games/guess-player/create-room", Icon: CreateRoomIcon },
  { label: "دخول غرفة", href: "/games/guess-player/rooms", Icon: JoinRoomIcon },
  { label: "الإحصائيات", href: "/games/guess-player/stats", Icon: StatsIcon },
  { label: "دليل اللعب", href: "/games/guess-player/guide", Icon: GuideIcon },
] as const;

const NAV = [
  { label: "الإحصائيات", href: "/games/guess-player/stats", Icon: StatsIcon, active: false },
  { label: "الرئيسية", href: "/", Icon: HomeIcon, active: true },
  { label: "دليل اللعب", href: "/games/guess-player/guide", Icon: GuideIcon, active: false },
] as const;
    ...
    <main className="relative mx-auto flex min-h-[100dvh] max-w-[26rem] flex-col overflow-hidden bg-[var(--lu-abyss)] px-5 pb-4 page-top">
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col">
        <header className="relative flex shrink-0 items-center justify-between gap-3 pt-1">
          <Link href={hubUrl} aria-label="رجوع للمنصة" className="lu-btn lu-frame grid size-10 place-items-center rounded-xl">
            <BackIcon size={20} />
          </Link>
        ...
        <section className="flex shrink-0 flex-col items-center py-2">
          ...
          <p className="mt-1 text-sm text-[var(--lu-tan)]">اسأل بنعم/لا واكشف اللاعب الخفي</p>
        </section>
        ...
        <Link href="/games/guess-player/rank" aria-label="التصنيف" className="lu-btn lu-frame flex shrink-0 items-center justify-between rounded-2xl px-5 py-3">
