...
export const metadata = { title: "دخول غرفة — خمن اللاعب" };
...
/** Live room occupancy from the Guess the Player game-server's in-memory
 *  state (mirrors Top Ten's read); renders empty on any failure. */
async function fetchRooms(): Promise<LiveRoom[]> {
  const base =
    process.env.GP_SERVER_INTERNAL_URL ??
    process.env.NEXT_PUBLIC_GP_SERVER_URL ??
    "http://localhost:4200";
  try {
