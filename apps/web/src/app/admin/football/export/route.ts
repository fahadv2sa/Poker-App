...
const SORTS = ["fame_desc", "fame_asc", "name_asc", "tier_asc", "birth_desc", "birth_asc"];
...
function parse(sp: URLSearchParams): PlayerFilter {
  ...
  return {
    ...
    position: POSITIONS.includes(position ?? "") ? (position as PositionCode) : undefined,
    ...
    fameMax: num("fameMax"),
    club: get("club"),
    missing: MISSING.includes(missing ?? "") ? (missing as PlayerMissing) : undefined,
    sort: SORTS.includes(sort ?? "") ? (sort as PlayerSort) : undefined,
  };
}
