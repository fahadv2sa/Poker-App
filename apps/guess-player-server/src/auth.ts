 ...
 * contract, PLATFORM_CONTRACTS §3). The Guess the Player server never trusts a client
 * identity: it verifies the signed token the Guess the Player web minted from the verified
...
function secretKey(): Uint8Array {
  ...
  if (!secret) {
    throw new Error(
      "AUTH_SECRET is not set on the Guess the Player server — it must match the web app's AUTH_SECRET.",
    );
  }
